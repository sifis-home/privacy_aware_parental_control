...
def main():
    ...
    for img in image_generator:
        ...
        input_img = cv2.cvtColor(cv2.GaussianBlur(img,(privacy_parameter,privacy_parameter),0), cv2.COLOR_BGR2RGB)
        private_img = cv2.cvtColor(cv2.GaussianBlur(img,(privacy_parameter,privacy_parameter),0), cv2.COLOR_BGR2RGB)
        ...
        if len(detected) > 0:
            for i, d in enumerate(detected):
                ...
                faces[i] = cv2.resize(private_img[yw1:yw2 + 1, xw1:xw2 + 1], (img_size, img_size))
            ...
            for i, d in enumerate(detected):
                label = "{}, {}".format(predicted_ages2[i],
                                        "M" if predicted_genders[i][0] < 0.5 else "F")
                draw_label(private_img, (d.left(), d.top()), label)

        # cv2.imshow("result", img)
        plt.imshow(private_img, cmap = 'gray', interpolation = 'bicubic')
